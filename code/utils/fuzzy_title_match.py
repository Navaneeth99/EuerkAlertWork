...
Scoped to the union of journal / institution / publisher hits for each PR
(from scope_entity_hits) and an optional publication-date window.
Runs after exact title, before embedding.
...
def _unmatched_prs(con):
    """PRs not DOI/exact-title matched, one row per scoped entity + Article Title."""
    con.execute(f"""
        ...
        FROM pr_base pr
        JOIN scope_entity_hits h ON pr."PR ID" = h."PR ID"
        WHERE CAST(pr."PR ID" AS VARCHAR) NOT IN (SELECT pr_id FROM doi_match)
          AND CAST(pr."PR ID" AS VARCHAR) NOT IN (SELECT pr_id FROM title_match)
...
def run_fuzzy_title_match(con, min_score: float = MIN_SCORE, max_days: int = MAX_DAYS) -> int:
    ...
    A PR may appear under multiple entities; ``match_fuzzy`` later keeps the
    highest-scoring hit per PR.
    """
    ...
    print(
        ...
        f"{len(pr_df):,} PR-entity rows, {len(doi_df):,} DOIs "
